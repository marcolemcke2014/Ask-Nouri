...
export default function StepEnergy({ user }: StepEnergyProps) {
  const router = useRouter();
  
  // State for user selections
  const [energyIssue, setEnergyIssue] = useState<string | null>(null);
  ...
  // Toggle a time of day selection
  const toggleTimeOfDay = (time: string) => {
    if (timeOfDay.includes(time)) {
      ...
      // Add the time to selected times
  ...
  const handleNext = async () => {
    try {
      ...
      const dataToSave = {
        ...
        energy_issue: energyIssue,
        energy_time_of_day: timeOfDay,
      ...
      if (!response.ok) {
        throw new Error(result.error || `Failed to save energy preferences (HTTP ${response.status})`);
      }
      ...
      // Add energy-specific parameters
      if (energyIssue) {
        params.append('energyIssue', energyIssue);
      }
      
      if (timeOfDay.length > 0) {
        params.append('timeOfDay', timeOfDay.join(','));
      }
    ...
    <OnboardingLayout currentStep={5}>
      <div className="bg-white rounded-lg p-6 shadow-sm text-gray-900">
        <h1 className="text-xl font-bold mb-6 text-center">
          Let's boost your energy and focus
        </h1>

        {/* Energy Issue Question */}
        <div className="mb-8">
          <h2 className="text-base font-medium mb-3">What's your main energy challenge?</h2>
          <div className="space-y-2">
            <div 
              className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors
                ${energyIssue === 'afternoon_slump' 
                  ? 'border-green-500 bg-green-50' 
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setEnergyIssue('afternoon_slump')}
            >
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center mr-3
                ${energyIssue === 'afternoon_slump' 
                  ...
                  : 'border-gray-400'}`}
              >
                {energyIssue === 'afternoon_slump' && <div className="h-2 w-2 rounded-full bg-green-500"></div>}
              </div>
              <span>Afternoon slump/crash</span>
            </div>
            
            <div 
              className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors
                ${energyIssue === 'general_fatigue' 
                  ? 'border-green-500 bg-green-50' 
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setEnergyIssue('general_fatigue')}
            >
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center mr-3
                ${energyIssue === 'general_fatigue' 
                  ? 'border-green-500' 
                  : 'border-gray-400'}`}
              >
                {energyIssue === 'general_fatigue' && <div className="h-2 w-2 rounded-full bg-green-500"></div>}
              </div>
              <span>General fatigue throughout day</span>
            </div>
            
            <div 
              className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors
                ${energyIssue === 'morning_fog' 
                  ? 'border-green-500 bg-green-50' 
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setEnergyIssue('morning_fog')}
            >
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center mr-3
                ${energyIssue === 'morning_fog' 
                  ? 'border-green-500' 
                  : 'border-gray-400'}`}
              >
                {energyIssue === 'morning_fog' && <div className="h-2 w-2 rounded-full bg-green-500"></div>}
              </div>
              <span>Morning brain fog</span>
            </div>
            
            <div 
              className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors
                ${energyIssue === 'post_meal_crash' 
                  ? 'border-green-500 bg-green-50' 
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setEnergyIssue('post_meal_crash')}
            >
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center mr-3
                ${energyIssue === 'post_meal_crash' 
                  ? 'border-green-500' 
                  : 'border-gray-400'}`}
              >
                {energyIssue === 'post_meal_crash' && <div className="h-2 w-2 rounded-full bg-green-500"></div>}
              </div>
              <span>Crash after meals</span>
            </div>
            
            <div 
              className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors
                ${energyIssue === 'focus_issue' 
                  ? 'border-green-500 bg-green-50' 
                  : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'}`}
              onClick={() => setEnergyIssue('focus_issue')}
            >
              <div className={`h-4 w-4 rounded-full border flex items-center justify-center mr-3
                ${energyIssue === 'focus_issue' 
                  ? 'border-green-500' 
                  : 'border-gray-400'}`}
              >
                {energyIssue === 'focus_issue' && <div className="h-2 w-2 rounded-full bg-green-500"></div>}
              </div>
              <span>Trouble focusing on tasks</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8">
          <h2 className="text-base font-medium mb-3">When do you most need an energy boost? (Select all that apply)</h2>
          <div className="flex flex-wrap gap-2">
            {['Morning', 'Midday', 'Afternoon', 'Evening', 'Pre-workout'].map((time) => (
              <div 
                key={time}
                onClick={() => toggleTimeOfDay(time.toLowerCase())}
                className={`px-3 py-2 rounded-full cursor-pointer text-sm transition-colors ${
                  timeOfDay.includes(time.toLowerCase())
        ...
        <div className="flex space-x-3 mt-8">
          ...
          <button
            onClick={handleNext}
            disabled={!energyIssue || isLoading}
